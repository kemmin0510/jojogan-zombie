            ...
            steps {
                script {
                    container('helm') {
                        ...
                        sh("helm repo update")
                        sh("helm upgrade --install node-exporter prometheus-community/prometheus-node-exporter --namespace kube-metrics -f $SCRIPT_DIR/node_exporter/node-exporter-values.yaml")
